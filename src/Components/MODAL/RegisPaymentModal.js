...
const RegisPaymentModal = ({appeal,setAppeal,RoundName, eventType, eventId, modelName, isShowPaymentComp, setIsShowPaymentComp, parentData, event_registration_id = null, notification_id = null, fee = null, end_time = null, event_type = null, start_time = null, fetchAllDataAfterPayment= null}) => {
  ...
  const onSubmit = (data) => {
    let aditionalData = {
      ...parentData,
      'event_registration_id': event_registration_id ? event_registration_id : null,
    ...
    axios.post(AppUrl.EventRegister, finalData, axiosConfig).then(res => {
      //console.log('res------',res)
      setBuffer(false)
  ...
  const modalButtonPress = () => {
    ...
    if (eventType == "OfflineMeetup") {
      ...
      //console.log('here---------')
      axios.get(AppUrl.CheckNotification, axiosConfig).then(res => {
            ...
            <View style={styles.topCard}>
              <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
                <Pressable onPress={() => setIsShowPaymentComp(false)}>
                  <Text style={{ color: '#000', fontWeight: 'bold', backgroundColor: '#ff0', paddingVertical: 4, paddingHorizontal: 8, borderRadius: 50, borderRadius: 10, marginHorizontal: 10, marginTop: 5 }}>X</Text>
                </Pressable>
              </View>
